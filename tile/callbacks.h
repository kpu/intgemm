...
class UnquantizeAndAddBiasAndWriteCallback {
  public:
    struct Config {
      float unquant_mult;
      const float* bias_addr;
      const float* c_begin_addr; // Need to determin column number for bias
    };

    // TODO: SLOW.  This is here for testing.
    template <Index A_rows, Index B_cols, typename Access> static void Run(Access access, const __m128i *from, const Config& config) {
      Slow<A_rows, B_cols>(access, reinterpret_cast<const typename Access::Content*>(from), config);
    }
    ...
    template <Index A_rows, Index B_cols, typename Access> static void Run(Access access, const __m512i *from, const Config& config) {
      RunImpl<A_rows, B_cols, B_cols>(access, from, config);
    }

  private:
    template <typename Access> static Index getCColumn(Access access, const Config& config) {
      return (&access.Front() - config.c_begin_addr) % access.Cols();
    }
    ...
    template <Index A_rows, Index B_cols, Index ColRemain, typename Access> INTGEMM_AVX512BW
      ...
      _mm512_storeu_ps(&access.Front(), kernels::add_bias(kernels::unquantize(*from, set1_ps<__m512>(config.unquant_mult)), config.bias_addr, getCColumn(access, config)));
      RunImpl<A_rows, B_cols, (ColRemain - 16)>(access.Add(0, 16), from + 1, config);
    }
    ...
    template <Index A_rows, Index B_cols, Index ColRemain, typename Access> INTGEMM_AVX512BW
      ...
      // Compilers seem to complain a lot about shifting past the end :-(
      constexpr __mmask16 mask = (remaining >= 16) ? 0xffff : (static_cast<__mmask16>(1 << remaining) - 1);
      _mm512_mask_i32scatter_ps(&access.Front() - (B_cols - ColRemain), mask, offsets, kernels::add_bias(kernels::unquantize(*from, set1_ps<__m512>(config.unquant_mult)), config.bias_addr, getCColumn(access, config)), sizeof(int32_t));
      // We just wrote 16 values: ColRemain, the next row (all or partial), possibly the next etc.
      // 16 - ColRemain of the next row and whatever followed.
      constexpr Index Wrote = ((remaining < 16) ? remaining : 16);
      constexpr Index Position = (B_cols - ColRemain) + Wrote;
      // TODO: more testing on this.
      RunImpl<A_rows - (Position / B_cols), B_cols, B_cols - (Position % B_cols)>(access.Add(Position / B_cols, Position % B_cols - (B_cols - ColRemain)), from + 1, config);
    }
    ...
    template <Index A_rows, Index B_cols, Index ColRemain, typename Access> INTGEMM_AVX512BW
      static typename std::enable_if<(A_rows == 1) && B_cols && (ColRemain < 16 && ColRemain > 0)>::type
      RunImpl(Access access, const __m512i *from, const Config& config) {
      _mm512_mask_storeu_ps(&access.Front(), (1 << ColRemain) - 1, kernels::add_bias(kernels::unquantize(*from, set1_ps<__m512>(config.unquant_mult)), config.bias_addr, getCColumn(access, config)));
    }

    // Nothing to write.
    template <Index A_rows, Index B_cols, Index ColRemain, typename Access> INTGEMM_AVX512BW
      static typename std::enable_if<!A_rows || !B_cols>::type
      RunImpl(Access, const __m512i *, float, const Config&) {}

    template <Index A_rows, Index B_cols, typename Access> static void Slow(Access access, const typename Access::Content *from, const Config& config) {
      for (Index i = 0; i < A_rows; ++i) {
        for (Index j = 0; j < B_cols; ++j) {
          (&access.Front())[i * access.Cols() + j] = from[i * B_cols + j] * config.unquant_mult + config.bias_addr[j];
        }
      }
    }
};
